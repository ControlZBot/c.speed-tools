...
def _owner_simple_callback(name: str, owner_id: int):
    @owner_only(owner_id)
    async def callback(interaction: discord.Interaction) -> None:
        await interaction.response.send_message(f"c.speed-tools {name} executed", ephemeral=True)
...
async def setup(bot: commands.Bot) -> None:
    ...
    async def globalban_add(interaction: discord.Interaction, user_id: str, reason: str) -> None:
        ...
        success = len([result for result in results if result.ok])
        failed = len(results) - success
        await interaction.followup.send(
            f"c.speed-tools globalban_add done: success={success}, failed={failed}",
            ephemeral=True,
        )
    ...
    for name in [
        ...
        global_group.add_command(
            app_commands.Command(
                name=name,
                description=f"c.speed-tools {name}",
                callback=_owner_simple_callback(name, owner_id),
            )
        )
    ...
    for name in [
        ...
        "owner_feature_flags",
        "owner_db_health",
        "owner_run_migrations",
        "owner_cache_clear",
        "owner_guild_allow_add",
        "owner_guild_allow_remove",
        "owner_guild_allow_list",
        "owner_cmd_disable",
        "owner_cmd_enable",
        "owner_emergency_lockdown",
        "owner_emergency_unlock",
        "owner_version",
    ]:
        ops_group.add_command(
            app_commands.Command(
                name=name,
                description=f"c.speed-tools {name}",
                callback=_owner_simple_callback(name, owner_id),
